...
import { heneenProducts } from '@/data/heneenProducts';
...
class ProductStore {
  ...
  initializeProducts(): void {
    ...
    // Add Heneen products (always include them, they're exclusive to Heneen Collection)
    const heneenProductsArray = Array.isArray(heneenProducts) ? heneenProducts.map((p: any) => ({
      ...p,
      createdAt: new Date(p.createdAt || Date.now()),
      updatedAt: new Date(p.updatedAt || Date.now())
    })) : [];
    ...
    if (stored === null) {
      const initialProducts = folderProductsArray.length > 0 
        ? [...folderProductsArray, ...heneenProductsArray] 
        : [...getDemoProducts(), ...heneenProductsArray];
      localStorage.setItem(STORAGE_KEY, JSON.stringify(initialProducts));
    } else if (folderProductsArray.length > 0 || heneenProductsArray.length > 0) {
      try {
  ...
  private getProductsFromStorage(): Product[] {
    ...
    try {
      ...
      const heneenProductsArray = Array.isArray(heneenProducts) ? heneenProducts : [];
      return [...folderProductsArray, ...heneenProductsArray].map((p: any) => ({
        ...p,
        createdAt: new Date(p.createdAt || Date.now()),
        updatedAt: new Date(p.updatedAt || Date.now())
      }));
