...
const Shop = () => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    // Exclude Heneen Collection products from Shop All, New Arrivals, and Best Sellers
    // But allow them if Heneen is explicitly selected in collection filter
    const isHeneenSelected = selectedCollection === 'heneen-collection';
    if (!isHeneenSelected) {
      result = result.filter(product => product.collection !== 'heneen-collection');
    }

    // Search filter
    if (searchQuery) {
      const searchResults = ProductService.searchProducts(searchQuery);
      if (!isHeneenSelected) {
        result = searchResults.filter(product => product.collection !== 'heneen-collection');
      } else {
