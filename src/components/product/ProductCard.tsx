...
const ProductCardComponent = ({ product, index = 0, onAddToCart }: ProductCardProps) => {
  ...
  const allImages = firstVariant?.images && firstVariant.images.length > 0 
    ? firstVariant.images 
    : (product.images || []);
  // Filter out video files - only show images in ProductCard
  const displayImages = allImages.filter(img => {
    const lowerImg = img.toLowerCase();
    return !lowerImg.endsWith('.mp4') && !lowerImg.endsWith('.mov') && !lowerImg.endsWith('.webm');
  });
