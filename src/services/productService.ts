...
export class ProductService {
  ...
  private static loadCollections(): void {
    try {
      ...
      const heneenCollection = {
        id: 'heneen-collection',
        name: 'Heneen Collection',
        description: 'Premium custom-fit pieces crafted with attention to detail and premium quality materials. Made for you.',
        createdAt: new Date(),
        updatedAt: new Date()
      };
      ...
      if (stored) {
        ...
        // Ensure Heneen Collection always exists
        const hasHeneen = this.collections.some(c => c.id === 'heneen-collection');
        if (!hasHeneen) {
          this.collections.push(heneenCollection);
          this.saveCollections();
        } else {
          // Update existing Heneen collection to ensure it has correct data
          const heneenIndex = this.collections.findIndex(c => c.id === 'heneen-collection');
          if (heneenIndex !== -1) {
            this.collections[heneenIndex] = {
              ...this.collections[heneenIndex],
              ...heneenCollection,
              createdAt: new Date(this.collections[heneenIndex].createdAt),
              updatedAt: new Date()
            };
            this.saveCollections();
          }
  ...
  static getCollectionById(id: string): Collection | undefined {
    // Ensure Heneen collection is always available
    if (id === 'heneen-collection') {
